...
// VISTA
function pintarPacientes(matrizEntrada) {
    var contenido = "<h2>" + matrizEntrada.length + "</h2>";
    matrizEntrada.forEach(function (paciente) {
        contenido += `<article id="paciente${paciente.id}">
        <h3>${paciente.nombre} ${paciente.apellido}</h3>
        <ul>
            <li>Diagnostico: ${paciente.diagnostico}</li>
            <li>Edad: ${paciente.edad}</li>
            <li>Numero SS: ${paciente.numeroSegSoc}</li>
        </ul>
    </article>`
        seccionPacientes.innerHTML = (matrizEntrada.length = 0) ? "SIN RESULTADOS" : contenido;
    })
}

function mostrarPantalla(matrizEntrada) {
    var section = document.getElementById("pacientes");
    section.innerHTML = ""
    if (matrizEntrada == "") section.innerHTML = "SIN RESULTADOS"
    else {
        let i = 0;
